...
# 2 Set base directory (IE ~/*insert folder where project is stored on device/data)
...
# 3 Standardize data into regions
```{python}
...
```{python}
def states_if_needed():
    if STATES_SHP.exists():
        return

    if not STATES_DIR.exists():
        raise FileNotFoundError("Not found in project folder.")

    if not STATES_SHP.exists():
        raise FileNotFoundError("Shapefile not found within directory. Check structure.")
